...
pub fn encode_message(msg: Message) -> Vec<u8> {
	...
	bytes.extend_from_slice(&msg.size.to_le_bytes()); // Message size.
	bytes.extend_from_slice(&msg.id.to_le_bytes()); // Message ID.
	bytes.extend_from_slice(&msg.msg_type.to_le_bytes()); // Message type.
	bytes.extend_from_slice(msg.body.as_bytes()); // Message body.
	bytes.extend_from_slice(&[0, 0]); // Terminator.
